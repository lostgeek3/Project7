...
class AddCourseDialog extends StatefulWidget {
  ...
  AddCourseDialog({required this.courseNotifier});
  @override
  _AddCourseDialogState createState() => _AddCourseDialogState();
}

final PickerKey1 = UniqueKey();
final PickerKey2 = UniqueKey();
final InputTitleKey = UniqueKey();
final InputTeacherKey = UniqueKey();
final InputLocationKey = UniqueKey();
final InputNoteKey = UniqueKey();
...
class _AddCourseDialogState extends State<AddCourseDialog> {
  ...
  /// 创建输入框
  Widget buildInputField({required String title, required bool isRequired, required TextEditingController controller, required UniqueKey key}) {
  ...
  Widget build(BuildContext context) {
    /// 判断添加的课程是否与已有课程时间冲突
    var courseSet = widget.courseNotifier.courses;
        ...
        child: Form(
          ...
          child: ListBody(
            children: [
              ...
              const SizedBox(height: 10),
              buildInputField(title: '课程名称', isRequired: true, controller: _courseNameController, key: InputTitleKey),
              const SizedBox(height: 10),
              ...
              buildInputField(title: '老师', isRequired: false, controller: _teacherController, key: InputTeacherKey),
              const SizedBox(height: 10),
              buildInputField(title: '地点', isRequired: false, controller: _locationController, key: InputLocationKey),
              const SizedBox(height: 10),
              buildInputField(title: '备注', isRequired: false, controller: _noteController, key: InputNoteKey),
            ],
          ),
        ),
      ),
          ...
          onPressed: () {
              if (_formKey.currentState!.validate()) {
                ...
                if (!isTimeConflict(newCourse)) {
                  widget.courseNotifier.addCourse(newCourse);
                  Navigator.of(context).pop();
                }
                else {
                  showDialog(
